...
@login_required
@email_verified
def lp_start(request, questionpaper_id=None):
    ...
    try:
        ...
        return 'Quiz not found, please contact your '\
    ...
    return render(request, 'yaksh/intro.html', context)
...
@login_required
@email_verified
def show_all_questions2(request):
    ...
    if request.method == 'POST':
        if request.POST.get('upload') == 'upload':
            ...
            if form.is_valid():
                try:
                    modules_with_questions_file = pd.read_excel(request.FILES['file'])
                except:
